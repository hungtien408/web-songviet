...
public partial class san_pham_ung_dung : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["ud"]))
            {
                var oProduct = new TLLib.Product();
                var dv = oProduct.ProductSelectOne(Request.QueryString["ud"]).DefaultView;
